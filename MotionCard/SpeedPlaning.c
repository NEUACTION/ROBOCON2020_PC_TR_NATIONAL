...
{
	return Pulse2Vel(MAX_MOTOR_SPEED);
}
...
void SpeedPlaning(void)
{
	...
	{
		//v ^ 2 = a * R 
		vell[i] = sqrt((1.0f * GetAccLimit()) * curvature[i]);
	...
	for (int i = 0; i < n - 1; i++)
	{
		...
		if (vell[i + 1] > vell[i])
		{
			...
			tempVirtualVel = sqrt(2 * GetAccLimit() * (GetRingBufferPointLen(i + 2) - GetRingBufferPointLen(i + 1)) + vell[i] * vell[i]);
			//该段轨迹结束的最大速度
	...
	for (int i = n - 1; i > 0; i--)
	{
		...
		{		
			...
			tempVirtualVel = sqrt(2 * GetAccLimit() * (GetRingBufferPointLen(i + 1) - GetRingBufferPointLen(i)) + vell[i] * vell[i]);
			//该段轨迹结束的最大速度
			tempTargetVel = vell[i - 1];
			//如果估算速度小于最大速度，用估算速度来估算法向加速度
			if(tempVirtualVel<vell[i - 1])
	...
	uint8_t ipointErr[5] = {0},ipointErrCount = 0;
	...
	//								(uint8_t *)"%d %d %d %d %d %d %d %d %d %d %d %d\r\n",ipoint,(int)lll,(int)vvv,(int)(time*1000),(int)a1,(int)a2,(int)a3,Err_ipoint[0],Err_ipoint[1],Err_ipoint[2],Err_ipoint[3],Err_ipoint[4]);
				...
				if ( (((a1 > GetAccLimit()) && (wheelOne[ipoint - 1] * wheelOne[ipoint - 2] > 0))\
					|| ((a2 > GetAccLimit()) && (wheelTwo[ipoint - 1] * wheelTwo[ipoint - 2] > 0))\
					|| ((a3 > GetAccLimit()) && (wheelThree[ipoint - 1] * wheelThree[ipoint - 2] > 0)))\
	...
	//			SetRingBufferPointVell(n, 100);
				for (int i = 0; i < n - 1; i++)
				{
					if (GetRingBufferPointVell(i + 2) > GetRingBufferPointVell(i + 1))
					{
						...
						//估算该段轨迹结束时的速度替代之前的最大速度
						tempVirtualVel = sqrt(2 * GetAccLimit() * (GetRingBufferPointLen(i + 2) - GetRingBufferPointLen(i + 1)) + GetRingBufferPointVell(i + 1) *GetRingBufferPointVell(i + 1));
						//该段轨迹结束的最大速度
				...
				for (int i = n - 1; i > 0; i--)
				{
					if (GetRingBufferPointVell(i) > GetRingBufferPointVell(i + 1))
					{
						...
						//估算该段轨迹结束时的速度替代之前的最大速度
						tempVirtualVel = sqrt(2 * GetAccLimit() * (GetRingBufferPointLen(i + 1) - GetRingBufferPointLen(i)) + GetRingBufferPointVell(i + 1) * GetRingBufferPointVell(i + 1));
						//该段轨迹结束的最大速度
