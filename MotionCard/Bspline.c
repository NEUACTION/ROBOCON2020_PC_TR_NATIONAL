...
//					(uint8_t *)"%d %d %d %d\r\n",(int)controlPoint[i].x,(int)controlPoint[i+1].x,(int)controlPoint[i+2].x,(int)controlPoint[i+3].x);
	...
	// keyPoint[0].poseAngle =	0.f;
	// keyPoint[0].point =  (Point_t){1110.0	,	7703.6};
	
//	if(gRobot.walkStatus==throw2ndShagai)
//	
//		if(gRobot.courtID == BLUE_COURT)
//		{
//			keyPoint[0].poseAngle=90.f;
//		}
...
//	}
	...
	for(int i = 1; i < resultNum-1; i++)
	{
		...
		{
			if(fabsf((keyPoint[i].angle - keyPoint[i-1].angle))>5&&fabsf((keyPoint[i].angle - keyPoint[i+1].angle))>5)
			{
