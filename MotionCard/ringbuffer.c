...
//		int sub =0.0f;
	...
	if(SEND)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			printf("PL %d %d ",(int)ringBuffer[i].point.x,(int)ringBuffer[i].point.y);
			printf("%d %d ",(int)ringBuffer[i].angle*10,(int)(ringBuffer[i].poseAngle*10));	
			printf("%d %d ",(int)ringBuffer[i].curvatureR,(int)ringBuffer[i].length);
			printf("%d ",(int)ringBuffer[i].vellMax);
			printf("\n");

		}

		printf("%d %d L %d ",(int)GetRingBufferPoint(n).x,(int)GetRingBufferPoint(n).y,(int)GetLength());
