...
//对一个值进行限幅
void ValueSaturation(float *value , float max , float min)
{
	if(*value>max)
	{
		*value = max;
...
float ReturnSaturateValue(float value , float max , float min)
{
	if(value>max)
	{
		value = max;
...
float CalculateDisPointToLine3(Point_t point, PointU_t line)
{
	float dis, k, b;
